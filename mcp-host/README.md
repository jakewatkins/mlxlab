...
Environment variables in the config are expanded from the system environment or from a `.env` file:
...
The config loader will automatically look for a `.env` file in the same directory as `mcp.json` and load environment variables from it. Variables already set in the system environment take precedence over those in the `.env` file.
