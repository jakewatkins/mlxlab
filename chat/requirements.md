...
## Overview
This is a simple LLM chatbot that hosts an LLM and allows the user to interactively prompt it until they type "quit" or "bye". The chat bot is written in Python and uses Apple's MLX libraries.

## User Flow
To start the chat bot the user will enter `llmchat` followed by the name of the Huggingface LLM they want to use. The program loads the LLM, sets up the model and tokenizer and then asks the user to enter their prompt. The prompt ends with the user typing the enter key. The chat bot transforms the user's prompt into the message object and then generates the LLM's response and prints it on the screen. After the llm has responded the chat bot will put up another line for the user to enter another prompt. The line will be "prompt ->".
